...
export default function AccessGroupsPage() {
  ...
  return <div className="page-shell-wide page-stack text-foreground">
    <PageHeader eyebrow="Administration / access" title="Access groups" description="Manage the explicit access groups used by article ACLs and cloud-provider mappings." icon={ShieldCheck} actions={<Button type="button" variant="secondary" size="sm" onClick={() => void load()} icon={<RefreshCw size={14} />}>Refresh</Button>} />
    <form onSubmit={create} className="glass-panel flex flex-col gap-3 rounded-2xl border border-border p-4 sm:flex-row sm:items-end">
      <div className="min-w-0 flex-1">
        <Input label="New access group" value={name} onChange={event => setName(event.target.value)} placeholder="e.g. Finance reviewers" />
      </div>
      <Button type="submit" variant="primary" disabled={busy === 'create'} loading={busy === 'create'} icon={<Plus size={15} />}>Create group</Button>
    </form>
