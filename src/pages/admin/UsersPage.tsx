...
export default function UsersPage() {
  ...
  const changeRoles = async (employee: ManagedUser, roleId: string) => {
    ...
    if (!next.size) { setError(`${employee.name} must keep at least one role. Assign the replacement role first, then remove this one.`); return }
  ...
  return <div className="page-shell-wide page-stack text-foreground">
    <header className="page-hero glass-panel rounded-panel border border-border px-4 py-5 shadow-[0_14px_34px_rgb(var(--shadow)/.1)] sm:px-6 sm:py-6"><div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-caption font-bold uppercase tracking-[.16em] text-muted-foreground">Administration / people</p><h1 className="mt-2 text-2xl font-semibold tracking-[-.04em] text-foreground sm:text-3xl">People &amp; access</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">One user can belong to many departments and hold many roles. Each department has one accountable owner.</p></div><Button variant="primary" size="md" icon={<Plus size={16} />} onClick={() => setInviteOpen(current => !current)}>{inviteOpen ? 'Close invite' : 'Invite employee'}</Button></div><div className="relative mt-5 grid grid-cols-2 gap-3 border-t border-border pt-4 sm:grid-cols-4"><Stat label="People" value={stats.total} icon={Users} /><Stat label="Active" value={stats.active} icon={Check} /><Stat label="Inactive" value={stats.inactive} icon={UserMinus} /><Stat label="Companies" value={stats.companies} icon={Building2} /></div></header>
    {inviteOpen && <form onSubmit={create} className="glass-panel rounded-panel space-y-4 border border-border p-4 sm:p-5"><div><h2 className="text-sm font-bold uppercase tracking-[.14em] text-muted">Invite employee</h2><p className="mt-1 text-xs text-muted-foreground">They will receive an email and sign in with Microsoft. Invitations expire after 7 days.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div><Input label="Full name" required value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder="Jane Doe" /></div><div><Input label="Work email" required type="email" value={form.email} onChange={event => setForm(current => ({ ...current, email: event.target.value }))} placeholder="jane@company.com" /></div><label className="block"><span className="label">Audiences</span><select multiple value={form.department_ids} onChange={event => setForm(current => ({ ...current, department_ids: Array.from(event.target.selectedOptions, option => option.value) }))} className="field mt-1.5 h-24">{departments.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="block"><span className="label">Role</span><Select value={form.role} onChange={event => setForm(current => ({ ...current, role: event.target.value }))} className="mt-1.5">{employeeRoles.map(role => <option key={role} value={role}>{role}</option>)}</Select></label></div><div className="flex items-center justify-end gap-2"><Button variant="secondary" size="md" onClick={() => setInviteOpen(false)}>Cancel</Button><Button variant="primary" size="md" icon={<Mail size={15} />} type="submit" loading={actingId === 'invite'}>{actingId === 'invite' ? 'Sending…' : 'Send invitation'}</Button></div></form>}
    {invitations.some(item => item.status === 'pending') && <section className="rounded-xl border border-info/20 bg-info/5 p-4"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-sm font-bold">Pending invitations</h2><p className="mt-1 text-xs text-muted-foreground">Invitations expire after seven days; resend to renew them.</p></div><Mail size={17} className="text-info" /></div><div className="space-y-2">{invitations.filter(item => item.status === 'pending').map(item => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card px-3 py-2.5"><div><p className="text-sm font-semibold">{item.name}</p><p className="text-xs text-muted-foreground">{item.email} · expires {formatDay(item.expires_at)}</p></div><div className="flex gap-2"><Button variant="secondary" size="sm" loading={actingId === item.id} onClick={() => void resend(item)}>Resend</Button><Button variant="danger" size="sm" loading={actingId === item.id} onClick={() => void revoke(item)}>Revoke</Button></div></div>)}</div></section>}
    {(message || error) && <div role="status" className={`flex items-center justify-between gap-3 rounded-xl border px-4 py-3 text-sm ${error ? 'border-destructive/25 bg-destructive/10 text-destructive' : 'border-success/25 bg-success/10 text-success'}`}><span>{error || message}</span><Button variant="ghost" size="sm" aria-label="Dismiss message" onClick={() => { setMessage(''); setError('') }} icon={<X size={15} />} /></div>}
    <section className="rounded-xl border border-border bg-card p-3 sm:p-4"><div className="flex flex-col gap-3 lg:flex-row"><div className="relative flex-1"><Input leftIcon={<Search size={16} />} value={query} onChange={event => setQuery(event.target.value)} placeholder="Search people, departments, or roles…" /></div><Select value={companyFilter} onChange={event => setCompanyFilter(event.target.value)} className="sm:w-52"><option value="all">All companies</option>{companies.map(([domain]) => <option key={domain} value={domain}>{domain}</option>)}</Select><Select value={statusFilter} onChange={event => setStatusFilter(event.target.value as typeof statusFilter)} className="sm:w-40"><option value="active">Active</option><option value="all">All status</option><option value="inactive">Inactive</option></Select><Button variant="secondary" size="md" icon={<RefreshCw size={15} />} onClick={() => void load()} className="h-10 w-10 shrink-0 !px-0" /></div></section>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(220px,.8fr)_minmax(0,1.8fr)_auto] xl:items-start">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {employee.role === 'CEO' && <Badge variant="info" size="sm" className="uppercase tracking-wide">CEO</Badge>}
          </div>
          <p className="mt-1 truncate text-xs text-muted-foreground">{employee.email}</p>
          <div className="mt-3 space-y-2">
            <div>
              <p className="text-caption font-bold uppercase tracking-[.12em] text-muted">Member of</p>
              <div className="mt-1 flex flex-wrap gap-1.5">
                {(employee.departments || []).length ? (employee.departments || []).map(item => <Badge key={item.id} variant="default" size="sm">{item.name}</Badge>) : <span className="text-body-sm text-muted-foreground">No department membership</span>}
              </div>
            </div>
            <div>
              <p className="text-caption font-bold uppercase tracking-[.12em] text-muted">Owns</p>
              <div className="mt-1 flex flex-wrap gap-1.5">
                {(employee.owned_departments || []).length ? (employee.owned_departments || []).map(item => <Badge key={item.id} variant="primary" size="sm">{item.name}</Badge>) : <span className="text-body-sm text-muted-foreground">No owned departments</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid min-w-0 gap-3 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-primary/15 bg-primary/[.03] p-3">
          ...
          <p className="mt-2 text-body-sm text-muted-foreground">Removing ownership does not deactivate the department.</p>
          <Link to="/admin/departments" className="mt-1 inline-flex text-body-sm font-semibold text-primary hover:underline">Manage department status →</Link>
        </div>
        <div className="flex flex-wrap gap-2 md:col-span-2">
          <p className="mr-1 basis-full text-caption font-bold uppercase tracking-[.12em] text-muted">Roles</p>
      ...
      <div className="flex shrink-0 items-start justify-start xl:justify-end">
        {employee.active ? <Button variant="danger" size="sm" icon={<UserMinus size={14} />} title={employee.id === currentUserId ? 'You cannot deactivate yourself' : `Deactivate ${employee.name}`} disabled={employee.id === currentUserId || actingId === employee.id} loading={actingId === employee.id} onClick={() => void onDeactivate(employee)}>Deactivate</Button> : <Button variant="secondary" size="sm" icon={<UserPlus size={14} />} disabled={actingId === employee.id} loading={actingId === employee.id} onClick={() => void onReactivate(employee)} className="border-success/20 text-success">Reactivate</Button>}
      </div>
    </div>
  </article>
}
...
function RoleToggle({ employee, role, isAdmin, busy, onChange }: { employee: ManagedUser; role: ManagedRole; isAdmin: boolean; busy: boolean; onChange: (employee: ManagedUser, roleId: string) => Promise<void> }) {
  ...
  return <label className={`inline-flex cursor-pointer items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-body-sm font-semibold ${assigned ? 'border-primary/25 bg-primary/10 text-primary-text' : 'border-border bg-surface text-muted-foreground'} ${disabled ? 'cursor-not-allowed opacity-50' : ''}`}><input type="checkbox" className="sr-only" checked={assigned} disabled={disabled} onChange={() => void onChange(employee, role.id)} /><span className={`grid h-3.5 w-3.5 place-items-center rounded border ${assigned ? 'border-primary bg-primary text-primary-foreground' : 'border-muted bg-input'}`}>{assigned && <Check size={10} strokeWidth={3} />}</span>{role.name}{role.active === false && <Badge variant="danger" size="sm" className="ml-1">inactive</Badge>}</label>
}
