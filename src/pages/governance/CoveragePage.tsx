...
// Mirrors src/domain/ai_service.py::FACT_TAXONOMY -- a lookup over the same small, fixed
// label set the backend's contradiction detection produces, not a general classifier.
// "other" (an unrecognised or not-yet-classified label) renders no badge at all rather
// than a misleading generic one.
...
export default function CoveragePage() {
  ...
  return <div className="page-shell page-stack">
    ...
    <section className="glass-panel rounded-2xl border border-border p-5"><h2 className="font-display text-lg font-bold text-foreground">Conflict resolution</h2><div className="mt-4 space-y-3">{conflicts.length ? conflicts.map((item) => <div key={item.id} className="rounded-xl border border-warning/20 bg-warning/5 p-4"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-foreground">{item.fact}</p>{CONTRADICTION_TYPE_LABELS[item.contradiction_type] && <Badge variant="warning" size="sm">{CONTRADICTION_TYPE_LABELS[item.contradiction_type]}</Badge>}</div><p className="mt-1 text-xs text-muted">{item.article_ids.length} conflicting articles</p><Button variant="primary" size="sm" icon={<CheckCircle2 size={14} />} onClick={async () => { const note = await dialog.prompt('How was this conflict resolved?', { title: 'Resolve conflict', confirmLabel: 'Resolve', placeholder: 'Resolution note' }); if (note) { await resolveConflict(item.id, note); await load() } }}>Mark resolved</Button></div>) : <p className="text-sm text-muted">No unresolved conflicts.</p>}</div></section>
  </div>
}
