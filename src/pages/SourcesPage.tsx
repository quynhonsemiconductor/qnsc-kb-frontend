...
export default function SourcesPage() {
  ...
  if (loadError && !result) return <div className="page-shell page-stack"><div role="alert" className="rounded-xl border border-destructive/25 bg-destructive/10 px-4 py-3 text-sm text-destructive">Failed to load sources. <Button variant="ghost" size="sm" className="ml-2 font-bold underline" onClick={() => void loadPage()}>Retry</Button></div></div>
  ...
  return <div className="page-shell page-stack">
    <PageHeader eyebrow="Traceability" title="Sources & files" description="Search and inspect the original files connected to each document without loading the entire source registry." icon={FolderOpen} actions={<div className="flex flex-wrap items-center justify-end gap-2">
      <form onSubmit={event => { event.preventDefault(); updateParams({ q: searchInput.trim() || undefined, offset: undefined }) }} className="flex items-center rounded-full border border-border bg-surface/70 pl-3 focus-within:border-primary/50">
        <Search size={14} className="text-muted" /><input aria-label="Search sources" value={searchInput} onChange={event => setSearchInput(event.target.value)} placeholder="Search files or documents…" className="w-40 bg-transparent px-2 py-1.5 text-xs text-foreground outline-none placeholder:text-muted sm:w-56" /><Button type="submit" variant="primary" size="sm" className="rounded-full">Search</Button>
      </form>
      <Badge variant="default" size="md">{pageRange}</Badge>
    </div>} />

    {loadError && <div role="alert" className="rounded-xl border border-destructive/25 bg-destructive/10 px-4 py-3 text-sm text-destructive">Could not refresh the source registry. <Button variant="ghost" size="sm" className="ml-2 font-bold underline" onClick={() => void loadPage()}>Retry</Button></div>}
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4"><Button variant="secondary" size="sm" disabled={result.offset <= 0} onClick={() => updateParams({ offset: String(Math.max(0, result.offset - PAGE_SIZE)) })} icon={<ArrowLeft size={14} />}>Previous</Button><span className="text-xs text-muted-foreground">Showing {pageRange}</span><Button variant="secondary" size="sm" disabled={result.offset + result.sources.length >= result.total} onClick={() => updateParams({ offset: String(result.offset + PAGE_SIZE) })} icon={<ArrowRight size={14} />} iconPosition="right">Next</Button></div>
  </div>
}
