...
export default function DialogProvider({ children }: { children: React.ReactNode }) {
  ...
  // Unique per provider instance, so the heading is referenced by id rather than a shared
  // hardcoded one that would collide with any other dialog mounted at the same time.
  const titleId = useId()
  // Escape is bound on the document, not on the overlay. As an onKeyDown on a non-focusable
  // div it only fired while focus happened to sit inside that subtree, so the key silently
  // stopped working as soon as focus moved anywhere else.
  useEffect(() => {
    if (!dialog) return
